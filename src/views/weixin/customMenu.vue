<template>
  <div>
      ...
      <div class="wechat-edit-bottom">
        ...
        <div class="wechat-edit-menu">
          ...
          <ul :style="{ width: widthThree + '%','margin-top':topThree + 'px'}">
            <li class="listThree" v-for='(item,index) in transverseObj.menuThree' @click="addMenuThree(index)">
              {{item}}
            </li>
          </ul>
        </div>
    ...
    <div class="wechat-dialog">
      ...
      <div class="menu-list">
        <label class="menu-list-input">菜单名称<input class="change-munu" v-on:keyup="goBlur()" type="text"
                                                  v-model="menuName"></label>
      ...
      <div v-if="isMenuContent">
        ...
        <div class="menu-url" v-if="radio==2?true:false">
          ...
          <label class="menu-url-input">页面地址<input class="network" type="text" placeholder="https://www.baidu.com"
                                                   v-model="netData" v-on:change="goNet()"/></label>
  ...
  export default {
    ...
    methods: {
      ...
      addMenuOne(index) { // 给第一列主菜单添加子菜单的方法
        ...
        const aLi = document.getElementsByClassName('listOne') // 获取第一列主菜单所有的li，添加点击边框变色的效果
        const oLi = document.getElementsByTagName('li') // 获取所有的li，为了重置边框的颜色
        ...
        const changeInput = document.getElementsByClassName('change-munu')[0] // 获取到菜单名称input，以添加获取焦点事件
      ...
      addMenuTwo(index) {
        ...
        this.havContent = false
        const aLi = document.getElementsByClassName('listTwo')
        ...
        aLi[index].style.borderColor = '#da2337'
        this.whatMenu = 2
        const changeInput = document.getElementsByClassName('change-munu')[0]
      ...
      addMenuThree(index) {
        ...
        this.havContent = false
        const aLi = document.getElementsByClassName('listThree')
        const oLi = document.getElementsByTagName('li')
        for (let i = 0, length = oLi.length; i < length; i++) {
          oLi[i].style.borderColor = '#dcdcdc'
        }
      ...
      preservation() { // 保存提交
        ...
        const buttonObj = {}
        const oneLi = document.getElementsByClassName('listOne')
        const twoLi = document.getElementsByClassName('listTwo')
        const threeLi = document.getElementsByClassName('listThree')// 获取菜单dom,后面验证使用
        let flag = true // 验证的决定值
      ...
      getMenuArr() { // 获取菜单
        this.$http.get('/wx/mp/customMenu').then(res => {
          console.log(res)
          const resopnse = res.data.menu
          for (let i = 0, length = resopnse.button.length; i < length; i++) {
      ...
      menuContent() { // 菜单内容验证函数
        ...
        let len = 0// 输入的字节长度
        for (let i = 0, length = this.menuName.length; i < length; i++) {
          const c = this.menuName.charCodeAt(i)
          // 单字节加1
      ...
      //         const flag = re.test(this.netData);
      //         if(flag){
